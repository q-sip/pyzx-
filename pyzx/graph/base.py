...
import numpy as np
from typing_extensions import (  # type: ignore # https://github.com/python/mypy/issues/5753
    GenericMeta,
    Literal,
)
...
VT = TypeVar(
    "VT", bound=int
)  # The type that is used for representing vertices (e.g. an integer)
...
class BaseGraph(Generic[VT, ET], metaclass=DocstringMeta):
    ...
    def __init__(self) -> None:
        ...
        self.phase_index: Dict[VT, int] = (
            dict()
        )  # {vertex:index tracking its phase for phase teleportation}
        ...
        self.variable_types: Dict[str, bool] = (
            dict()
        )  # DEPRICATED - mapping of variable names to their type (bool or continuous)
    ...
    def compose(self, other: BaseGraph[VT, ET]) -> None:
        ...
        for k in range(len(outputs)):
            ...
            plugs.append(
                (
                    ...
                    ni,
                    EdgeType.HADAMARD
                    if self.edge_type(self.edge(no, o))
                    != other.edge_type(other.edge(i, ni))
        ...
        minr = min(
            (
                ...
                if other.type(v) != VertexType.BOUNDARY
            ),
            default=0,
        )
        offset = maxr - minr + 1
    ...
    def to_tensor(
        self, preserve_scalar: bool = True, strategy: str = "naive"
    ) -> np.ndarray:
    ...
    def to_matrix(
        ...
        return tensor_to_matrix(
            tensorfy(self, preserve_scalar, strategy),
            self.num_inputs(),
            self.num_outputs(),
        )
    ...
    def from_tikz(
        ...
        return tikz_to_graph(
            tikz, warn_overlap, fuse_overlap, ignore_nonzx, cls.backend
        )
    ...
    def save(self, filename: str, fmt: Optional[str] = None) -> None:
        ...
        if fmt == "json":
            ...
            raise ValueError(
                f"Unsupported format '{fmt}'. Supported formats are 'json' and 'tikz'."
            )
    ...
    @classmethod
    def load(
        cls, filename: str, fmt: Optional[str] = None, **kwargs: Any
    ) -> BaseGraph[VT, ET]:
    ...
    def is_id(self) -> bool:
        ...
        if (
            ...
            or self.num_vertices() != 2 * len(inputs)
            or self.num_edges() != len(inputs)
        ):
            return False

        for i in range(len(inputs)):
            if not self.connected(inputs[i], outputs[i]):
                return False
        return True
    ...
    def auto_detect_io(self):
        ...
        for v in self.vertices():
            ...
            if self.vertex_degree(v) != 1:
                raise TypeError(
                    "Invalid ZX-diagram: Boundary-type vertex does not have unique neighbor"
                )
            ...
            if self.row(w) > self.row(v):
                ...
                raise TypeError(
                    "Boundary-type vertex at same horizontal position as neighbor. Can't determine whether it is an input or output."
                )
        inputs = sorted(filter(lambda v: v in self.vertices(), inputs), key=self.qubit)
        outputs = sorted(
            filter(lambda v: v in self.vertices(), outputs), key=self.qubit
        )
    ...
    def add_edge_table(self, etab: Mapping[Tuple[VT, VT], List[int]]) -> None:
        ...
        for st, (ns, nh) in etab.items():
            for _ in range(ns):
                self.add_edge(st, EdgeType.SIMPLE)
            for _ in range(nh):
                self.add_edge(st, EdgeType.HADAMARD)
    ...
    def phase_negate(self, v: VT) -> None:
        if v not in self.phase_index:
            return
        index = self.phase_index[v]
        mult = self.phase_mult[index]
        if mult == 1:
            self.phase_mult[index] = -1
        else:
            self.phase_mult[index] = 1
    ...
    def remove_isolated_vertices(self) -> None:
        ...
        for v in self.vertices():
            ...
            if d == 1:  # It has a unique neighbor
                ...
                if self.type(v) == VertexType.BOUNDARY:
                    continue  # Ignore in/outputs
                w = list(self.neighbors(v))[0]
                if len(list(self.neighbors(w))) > 1:
                    continue  # But this neighbor has other neighbors
                if self.type(w) == VertexType.BOUNDARY:
                    continue  # It's a state/effect
                ...
                if t1 == VertexType.H_BOX:
                    t1 = VertexType.Z  # 1-ary H-box is just a Z spider
    ...
    def is_phase_gadget(self, v: VT) -> bool:
        ...
        if (
            not vertex_is_zx(self.type(v))
            or self.phase(v) != 0
            or self.vertex_degree(v) < 2
        ):
            return False
