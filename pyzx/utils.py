...
def phase_to_s(
    a: FractionLike,
    t: VertexType = VertexType.Z,
    poly_with_pi: bool = False,
    limit_denominator: bool = True,
) -> str:
...
def phase_fraction_to_s(
    a: FractionLike, t: VertexType = VertexType.Z, limit_denominator: bool = True
) -> str:
    if a == 0 and t != VertexType.H_BOX:
        return ""
    if a == 1 and t == VertexType.H_BOX:
        return ""
...
class Settings(object):  # namespace class
    ...
    topt_command: Optional[List[str]] = (
        None  # Argument-separated command to run TOpt such as ["wsl", "./TOpt"]
    )
...
try:
    ...
    if relpath.count("..") <= 2:  # We are *probably* working in the PyZX directory
        settings.javascript_location = os.path.relpath(
            settings.javascript_location, os.getcwd()
        )
...
def ave_pos(a, b, perc=1 / 2):
    return (abs(a - b)) * (perc) + min(a, b)
