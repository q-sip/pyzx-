...
HADAMARD_EDGE_CANCELLATION_MUTANT = """
// Mutant: WRONG ZX semantics.
// Should create a SIMPLE wire (t=1) for HH-cancel, but we create HADAMARD (t=2).

// Find all marked patterns (scoped to this graph_id)
MATCH (s:Node {graph_id: $graph_id})
WHERE s.pattern_id IS NOT NULL

WITH DISTINCT s.pattern_id AS pattern_id

// Reconstruct the full path for each pattern (within this graph)
MATCH path = (start:Node {graph_id: $graph_id})-[:Wire*2..6]-(end:Node {graph_id: $graph_id})
...
// Create direct connection (MUTANT: Hadamard instead of Simple)
WITH DISTINCT start, end, nodes_to_delete, pattern_id
CREATE (start)-[newWire:Wire {t: 2}]->(end)

// Delete the internal marked nodes (this will also delete the old marked wires via DETACH)
