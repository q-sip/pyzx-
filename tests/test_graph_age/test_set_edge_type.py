...
class TestGraphAGESetEdgeType(unittest.TestCase):
	...
	def test_set_edge_type_updates_edge(self):
		"""set_edge_type should update edge type value."""
	...
	def test_set_edge_type_works_with_reversed_edge(self):
		"""set_edge_type should work regardless of endpoint order."""
		v0, v1 = self.g.add_vertices(2)
		self._create_edge(v0, v1)

		self.g.set_edge_type((v1, v0), EdgeType.HADAMARD)
		self.assertEqual(self.g.edge_type((v0, v1)), EdgeType.HADAMARD)

	def test_set_edge_type_missing_edge_does_not_crash(self):
		"""set_edge_type on a missing edge should not raise."""
		v0, v1 = self.g.add_vertices(2)
		self.g.set_edge_type((v0, v1), EdgeType.HADAMARD)
