...
return count(center) AS patterns_processed"

-----------------------------------------------------------------------------------------------------

//Hadamard edge cancellation
...
//Pivot rule - single interior Pauli spider
...
WITH a, b, 
     ...
     COUNT(CASE WHEN coalesce(b_neighbor.t, -1) <> 0 THEN 1 END) as non_boundary_count,
...
WITH a, b, boundary_vertex,
     COUNT(CASE WHEN coalesce(all_a_edges.t, 1) <> 2 THEN 1 END) as non_t2_edges
...
SET new_connection.t = new_edge_type,
    new_connection.graph_id = coalesce(a.graph_id, 0)
// Remove b
DETACH DELETE b
