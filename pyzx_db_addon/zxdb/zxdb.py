...
class ZXdb:
    ...
    def _ensure_boundary_neighbors(self) -> int:
        """Attach a neutral Z spider to any disconnected boundary node.

        This prevents downstream normalize/extraction crashes that assume every
        boundary has at least one neighbor.
        """

        query = """
        MATCH (b:Node {graph_id: $graph_id, t: 0})
        WHERE NOT (b)-[:Wire]-()
        CREATE (z:Node {graph_id: $graph_id, t: 1, phase: 0.0})
        CREATE (b)-[:Wire {t: 1, graph_id: $graph_id}]->(z)
        RETURN count(b) AS repaired
        """

        with self.driver.session() as session:
            def apply_repair(tx):
                rec = tx.run(query, graph_id=self.graph_id).single()
                return int(rec["repaired"] or 0) if rec is not None else 0

            repaired = session.execute_write(apply_repair)
            if repaired:
                logging.warning(
                    "Repaired %s disconnected boundary node(s) for graph ID '%s'",
                    repaired,
                    self.graph_id,
                )
            return repaired
    ...
    def pivot_boundary_rule(self) -> int:
        ...
        with self.driver.session() as session:
            def apply_pivot_boundary_labeling(tx):
                before_bad = self._boundary_degree_violations_tx(tx, self.graph_id)
                changed = self._execute_rewrite_query(tx, pb_query, self.graph_id)
                if changed == 0:
                    return 0

                after_bad = self._boundary_degree_violations_tx(tx, self.graph_id)
                if after_bad > before_bad:
                    # Abort this transaction: boundary rewrite is not boundary-safe here.
                    raise RuntimeError("pivot_boundary_degree_violation")
            ...
            try:
                ...
                if str(exc) == "pivot_boundary_degree_violation":
                    logging.info(
                        "Skipping pivot boundary rewrite that violates boundary degree for graph ID '%s'",
                        self.graph_id,
                    )
                    return 0
                raise
    ...
    def full_reduce(
        ...
        max_main_iterations: int = 200,
        pivot_only_stall_limit: int = 6,
    ):
        # Backward compatibility: allow positional full_reduce(200, ...)
        if isinstance(graph, int):
            max_main_iterations = graph
            graph = None

        if graph is not None and hasattr(graph, "graph_id"):
            self.graph_id = graph.graph_id

        # Reindexing renumbers node ids in DB. When an external Graph* object is
        # passed in, that object still holds its original input/output ids, so
        # renumbering here can leave stale boundary references for normalize().
        should_reindex = graph is None
        ...
        for _ in range(max_main_iterations):
            ...
            if not (i or k or l):
                if j == 0:
                    ...
                    self._ensure_boundary_neighbors()
                    if should_reindex:
